                                ...
                                double coefspe)
{
   ...
   else {_freqCallMin=0.0f; _exFreqCallMin=0.8f;}
...
bool DetecTreatment::openWavFile(QString& pathFile)
{
    ...
    // ajouter une condition ou passer coefspe dans version.ini
    //coefspe = 2.5d;
...
bool DetecTreatment::computeFFT(QString &wavFile)
{
    ...
    _detec->LogStream << "(computefft) samplerate = " << QString::number(_soundFileInfo.samplerate) << endl;
    ...
    _detec->LogStream <<  "LimY dans computefft = " << LimY << endl;
...
void DetecTreatment::correctNoise()
{
    ...
    _detec->LogStream << "LimY = " << LimY << endl;
    _detec->LogStream << "KhzPerY = " << KhzPerY << endl;
...
void DetecTreatment::detectsParameter2()
{
    ...
    double epsilon = 0.0001d;
    //
    if(_detec->IDebug) _detec->LogStream << "dp2-1" << endl ;
    int nbcris = CallsArray.size();
    ...
    for (int icri = 0 ; icri < nbcris ; icri++) //Execute for each call
    {
        ...
        if(_detec->IDebug) _detec->LogStream << "-403" << endl;  //+++
        ...
        if(_detec->IDebug) _detec->LogStream << "-404" << endl;  //+++
        ...
        double eTot = 0.0d;
        double eMoy = 0.0d;
        if(_detec->IDebug) _detec->LogStream << "-405" << endl;  //+++
        ...
        if(_detec->IDebug) _detec->LogStream << "-406" << endl;  //+++
        ...
        for(int k=0;k<=ymax-ymin;k++)
        {
            ...
            //if(xc>=xmin)
            //{
            if(_detec->IDebug) _detec->LogStream << "-406,1  xc-xmin = " << xc-xmin  <<
                               "   xc=" << xc << "   xmin=" << xmin << endl;  //+++
        ...
        if(_detec->IDebug) _detec->LogStream << "-407" << endl;  //+++
        int yfds = 0;
        int yfdm = 0;
        double recfds= 0.0d;
        double recfdm= 0.0d;
        ...
        if(_detec->IDebug) _detec->LogStream << "-408" << endl;  //+++
        int xpps = 0;
        int xppm = 0;
        double recpps= 0.0d;
        double recppm= 0.0d;
        ...
        if(_detec->IDebug) _detec->LogStream << "-409" << endl;  //+++
        ...
        if(_detec->IDebug) _detec->LogStream << "-419" << endl;  //+++
        double prorata,bdeb,bfin,larco,lartot;
        int ideb,ifin;
        if(_detec->IDebug) _detec->LogStream << "-6" << endl ;  //+++
        ...
        if(_detec->IDebug) _detec->LogStream << "-61" << endl ;  //+++
        ...
        if(_detec->IDebug) _detec->LogStream << "-62" << endl ;  //+++
        for(int inoise=0;inoise<4;inoise++)
        {
            int jdeb,jfin,x,y;
            int nps=0;
            double bruit=0.0d,bruit_moyen=0.0d;
            if(_detec->IDebug) _detec->LogStream << "-621 inoise=" << inoise << endl ;  //+++
            if(inoise<2) {jdeb=ymin;jfin=ymax;}
            else {jdeb=xmin;jfin=xmax;}

            if(_detec->IDebug) _detec->LogStream << "-622 jdeb=" << jdeb << " jfin=" << jfin << endl ;  //+++

            for(int j=jdeb;j<=jfin;j++)
            {
                if(_detec->IDebug) _detec->LogStream << "-623 j=" << j << endl ;  //+++

                x=j;y=j;
                for(int k=1;k<=3;k++)
                {
//                    if(_detec->IDebug) _detec->LogStream << "-624 j=" << j << "  k=" << k << endl ;  //+++
                    if((inoise<2 && _xMaxPerY[j-ymin]>0) || (inoise>1 &&  _yMaxPerX[j-xmin]>0))
                    {
//                        if(_detec->IDebug) _detec->LogStream << "-625 j=" << j << "  k=" << k << endl ;  //+++
                        if(inoise==0) x=_xMinPerY[j-ymin]-k;
                        if(inoise==1) x=_xMaxPerY[j-ymin]+k;
                        if(inoise==2) y=_yMinPerX[j-xmin]-k;
//                        if(_detec->IDebug) _detec->LogStream << "-626 j=" << j << "  k=" << k << endl ;  //+++
                        if(inoise==3) y=_yMaxPerX[j-xmin]+k;
//                        if(_detec->IDebug) _detec->LogStream << "-627 j=" << j << "  k=" << k << endl ;  //+++
                        if(x>=0 && x<SonogramWidth && y>0 && y<FftHeightHalf && y <MAXHEIGHT)
                        {
//                            if(_detec->IDebug) _detec->LogStream << "-6271 y=" << y << "  x=" << x << endl ;  //+++
                            bruit += (double)SonogramArray[y][x];
//                            if(_detec->IDebug) _detec->LogStream << "-6272 y=" << y << "  x=" << x << endl ;  //+++
                            nps++;
                        }
//                        if(_detec->IDebug) _detec->LogStream << "-6279 j=" << j << "  k=" << k << endl ;  //+++
                    }
                }
//                if(_detec->IDebug) _detec->LogStream << "-628 j=" << j << endl ;  //+++
            }
...
//        if(_detec->IDebug) _detec->LogStream << "-63" << endl ;  //+++
        ...
        //if(_detec->IDebug) _detec->LogStream << "-64" << endl ;  //+++
        double mdm = sdm/nms;
        double sdc = 0.0d;
        for(int k=0;k<xmax-xmin+1;k++)
        {
        ...
        //if(_detec->IDebug) _detec->LogStream << "-65" << endl ;  //+++
        ...
        //if(_detec->IDebug) _detec->LogStream << "-66" << endl ;  //+++
        ...
        int np,y,xn,nn,yn;
        amp[0]=0.0d;
        //if(_detec->IDebug) _detec->LogStream << "-67" << endl ;  //+++
        ...
        //if(_detec->IDebug) _detec->LogStream << "-68" << endl ;  //+++
        ...
        //if(_detec->IDebug) _detec->LogStream << "-69" << endl ;  //+++
        ...
        double e1,e2,e3;
        if(_detec->IDebug) _detec->LogStream << "-70" << endl ;  //+++
        ...
        if(_detec->IDebug) _detec->LogStream << "-71" << endl ;  //+++
        ...
        if(_detec->IDebug) _detec->LogStream << "-72" << endl ;  //+++
        ...
        if(_detec->IDebug) _detec->LogStream << "-73" << endl ;  //+++
        ...
        // -----------------------------------------------------------------------------------
        if(_detec->IDebug) _detec->LogStream << "-8" << endl ;  //+++
        oParam[Dbl8] = 0.0f;
        double e8 = 0.0d;
        int n8 = 0;
    ...
    if(_detec->IDebug) _detec->LogStream << "-10" << endl;
    int nhsup,nhinf;
...
{
    _detec->LogStream << "debut saveparameters" << endl;
    double epsilon = 0.0001d;
    ...
    _detec->LogStream << "saveparameters 2" << endl;
    ...
    _detec->LogStream << "saveparameters 3" << endl;
    txtFile.close();
}
